...
export default function ProfileNavComponent(props: Props) {
	...
	return (
		<div className="nav-container">
			...
			>
				<div className="nav-label">incoming</div>
				{props.selection === 3 ? <div className="nav-indicator"></div> : <></>}
			</div>
			<div
				className="nav-item"
				onClick={() => {
					props.selectionChanged(4);
				}}
			>
				<div className="nav-label">outgoing</div>
				{props.selection === 4 ? <div className="nav-indicator"></div> : <></>}
			</div>
			<div
				className="nav-item"
				onClick={() => {
					props.selectionChanged(5);
				}}
			>
				<div className="nav-label">blocked</div>
				{props.selection === 5 ? <div className="nav-indicator"></div> : <></>}
			</div>
		</div>
	);
}
